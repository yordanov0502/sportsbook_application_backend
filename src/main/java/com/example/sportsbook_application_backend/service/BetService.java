...
public class BetService {
    ...
    public void callAPIForOddsByDate(String date){
        ...
        for(Event event : events)
        {
            ...
            {
                Bet betHome = new Bet(null, event, Outcome.PENDING, Type.ONE,Float.parseFloat(odds.get(event.getId()).getBookmakers().get(0).getBets().get(0).getValues().get(0).getOdd()));
                Bet betDraw = new Bet(null, event,Outcome.PENDING, Type.ZERO,Float.parseFloat(odds.get(event.getId()).getBookmakers().get(0).getBets().get(0).getValues().get(1).getOdd()));
                Bet betAway = new Bet(null, event,Outcome.PENDING, Type.TWO,Float.parseFloat(odds.get(event.getId()).getBookmakers().get(0).getBets().get(0).getValues().get(2).getOdd()));
